    ...
    <div className="fixed bottom-0 left-0 right-0 z-50 bg-brand-navy text-white shadow-2xl">
      <div className="mx-auto max-w-[1240px] px-4 sm:px-6 py-3">
        ...
        <div className="sm:hidden">
          {/* Row 1: image + name/price + bundles */}
          <div className="flex items-center gap-3 mb-3">
            ...
            <div className="flex-1 min-w-0">
              ...
              <div className="flex items-center gap-2 text-sm">
                <span className="line-through text-white/50">{formatPrice(product.originalPrice)}</span>
                <span className="font-bold">{formatPrice(product.salePrice)}</span>
              </div>
            </div>
            <div className="relative shrink-0">
              <div className="pointer-events-none flex items-center gap-1.5 bg-gray-200 text-brand-dark font-bold text-xs rounded-xl px-3 py-2 whitespace-nowrap">
                BUNDLES
                <svg width="10" height="6" viewBox="0 0 10 6" fill="none">
                  <path d="M1 1L5 5L9 1" stroke="#333" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/>
                </svg>
              </div>
              <select value={bundle} onChange={(e) => onBundleChange(e.target.value)} className="absolute inset-0 opacity-0 cursor-pointer w-full">
          ...
          <button
            ...
            className="w-full bg-brand-yellow text-brand-dark font-bold rounded-full py-3 text-base hover:brightness-95 transition-all flex items-center justify-center gap-2"
          >
            Add to cart 🛒
          </button>
        </div>
        ...
        <div className="hidden sm:flex items-center gap-4">
          <div className="relative w-20 h-16 rounded-lg overflow-hidden bg-white/10 shrink-0">
            <Image src={product.images[0]} alt={product.name} fill className="object-cover" />
          </div>
          <div className="flex-1 min-w-0">
